...
switch ($_GET['do']) {
  ...
  case 'gt':
    ...
    $ary = array(0, 0, 0, 0, 0);
    foreach ($re as $ro)
      $ary[$ro['time']] += count(unserialize($ro['seat'])); //收集各時段已售出之量

    $now = (date("H") >= 14 && $today == $_POST['date']) ? floor(date("H") / 2 - 6) : 0;
    //(14點過後且是今天)?需特別處理得到1~5:從0;

    // floor(date("H") / 2 - 6) 
    //只有當天的可選時段會變化，可能是從0開始跑或從現在時間(轉成時段)開始跑
    //if 現在是pm3 -> 15/2-6=1(無條件捨去) -> se_time[1]=1600~1800

    for ($i = $now; $i < 5; $i++)  //$now只控制迴圈要跑 0~4 次 $i代表電影的第幾場次最小值0
      echo '<option value="' . $i . '">' . $seat[$i] . '剩餘座位' . (20 - $ary[$i]) . '</option>';
    break;
